...
[ave_sp.vertices,ave_sp.faces]=read_surf(['/home/ajoshi/freesurfer/subjects/fsaverage/surf/lh.sphere.reg']);ave_sp.faces=ave_sp.faces+1;
[ave.vertices,ave.faces]=read_surf(['/home/ajoshi/freesurfer/subjects/fsaverage/surf/lh.inflated']);ave.faces=ave.faces+1;
...
for jj=1:6
    ...
    for kk=1:length(ll1)
        ...
        for ii=1:length(ll2)
            ...
            for ww=1:min(1,length(subdir))
                ...
                if exist([subbasename2,'/surf/lh.sphere.reg'],'file') && exist([subbasename2,'/surf/lh.thickness'],'file')
                    %pth=fileparts(subbasename);
 
                    thicknessr(:,scanno1)=load_mgh([subbasename2,'/surf/lh.thickness.fsaverage.mgh']);
                    ...
                    saveas(h,sprintf('FS_sub%d_%d_l_1ge_vs_siemens_left2_1.png',jj,scanno1));
                    autocrop_img(sprintf('FS_sub%d_%d_l_1ge_vs_siemens_left2_1.png',jj,scanno1));
                    view(-90,0);camlight('headlight');material dull;
                    saveas(h,sprintf('FS_sub%d_%d_l_1ge_vs_siemens_left2_2.png',jj,scanno1));
                    autocrop_img(sprintf('FS_sub%d_%d_l_1ge_vs_siemens_left2_2.png',jj,scanno1));
    ...
    if scanno1>1
        ...
        view(90,0);camlight('headlight');material dull;
        saveas(h,sprintf('diff_thickness_FS_sub%d_l_1ge_vs_siemens_left2.png',jj));
        autocrop_img(sprintf('diff_thickness_FS_sub%d_l_1ge_vs_siemens_left2.png',jj));
        view(-90,0);camlight('headlight');material dull;
        saveas(h,sprintf('diff_thickness_FS_sub%d_l_2ge_vs_siemens_left2.png',jj));
        autocrop_img(sprintf('diff_thickness_FS_sub%d_l_2ge_vs_siemens_left2.png',jj));
