...
for jj=1:length(lst)
    ...
    if (exist(['/home/ajoshi/for_shravanr/',lst(jj).name,'/atlas.iso-thickness_0-6mm.left.mid.cortex.dfs'],'file') && exist([subbasename,'.left.mid.cortex.svreg.dfs'],'file'))  
       ...
       sl=readdfs(['/home/ajoshi/for_shravanr/',lst(jj).name,'/atlas.iso-thickness_0-6mm.left.mid.cortex.dfs']);       
       sr=readdfs(['/home/ajoshi/for_shravanr/',lst(jj).name,'/atlas.iso-thickness_0-6mm.right.mid.cortex.dfs']);       
...
save left_mapped2_right_fcon1000_iso;
...
save left_mapped2_right_fcon1000_iso_smooth;
...
pp(bvl)=1;
pfdr=FDR(pp,0.00000005); pp=pp.*0.00000005./pfdr;
...
patch('faces',smidltarc.faces,'vertices',smidltarc.vertices,'facevertexcdata',10*(0.00000005-pp1).*(pp1<0.00000005),'facecolor','interp','edgecolor','none');
