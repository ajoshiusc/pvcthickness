...
function thicknessPVCwmgm(varargin)
% Usage:
...
GMfrac(pvc.img>(2+Thr) & pvc.img<=3) = 3 - pvc.img(pvc.img>(2+Thr) & pvc.img<=3);
nii = pvc;
nii.img = GMfrac;
save_untouch_nii_gz(nii, [subbase '.GM_frac_Thr',num2str(Thr),'.nii.gz']);
GMfrac = GMfrac + eps;
clear nii pvc
...
save_untouch_nii_gz(nii, [subbase '.heat_map_sol_Thr',num2str(Thr),'.nii.gz'], 64);
...
writedfs([subbase, '.pvc-thickness_Thr',num2str(Thr),'_0-6mm.isosurface.dfs'], fv);
...
if isequal(size(inner.vertices), size(pial.vertices))
   ...
   writedfs([subbase, '.pvc-thickness_Thr',num2str(Thr),'_0-6mm.mid.cortex.dfs'], mid);
end

if exist([subbase, '.left.inner.cortex.svreg.dfs'],'file')
    split_thickness_map_Thr(subbase,Thr);
end

save([subbase, '.heat_sol_comp_Thr',num2str(Thr),'.mat'], 'volComp_x', 'volComp_y', 'volComp_z', 'heat_map', ...
