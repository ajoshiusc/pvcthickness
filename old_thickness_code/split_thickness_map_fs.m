...
function split_thickness_map_fs(subbase)

inner=readdfsGz([subbase, '.inner.cortex.dfs']);
pial=readdfsGz([subbase, '.pial.cortex.dfs']);
th=readdfsGz([subbase, '.inner.fs.cortex.dfs']);
...
writedfs([subbase, ['.left.mid.cortex.fs.dfs']],mid_l);
...
writedfs([subbase, ['.right.mid.cortex.fs.dfs']],mid_r);
