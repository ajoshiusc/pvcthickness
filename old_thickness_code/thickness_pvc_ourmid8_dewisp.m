...
function thickness_pvc_ourmid8_dewisp(subbasename,iso_heat,flag1)
if ~exist('iso_heat','var')
    iso_heat=0;
end
vv=load_nii([subbasename,'.bfc.nii.gz']);
vpvc=load_nii([subbasename,'.pvc.frac.nii.gz']);
vv_dewisp=load_nii([subbasename,'.cortex.dewisp.mask.nii.gz']);
temp = imgaussian(double(vv_dewisp.img>0), 0.7);
%msk_inner_vol = interpn( temp, volComp_x, volComp_y, volComp_z, 'linear', 0);
pix = 2; [x1,y1,z1] = ndgrid(-pix:pix);
se2 = (sqrt(x1.^2 + y1.^2 + z1.^2) <=pix);
vv_dewisp.img = imerode(vv_dewisp.img>0.5, se2);
...
vv_dewisp=resample_avw(vv_dewisp,[512 512 512],'nearest');
%view_nii(vv);
...
if iso_heat==0
pialinnerdist=spial.vertices-sin.vertices;
pialinnerdist=pialinnerdist./repmat(sqrt(sum(pialinnerdist.^2,2)),1,3);
if exist('flag1','var')
sin.vertices=sin.vertices-2*pialinnerdist;
spial.vertices=spial.vertices+2*pialinnerdist;
else
sin.vertices=sin.vertices-pialinnerdist;
%spial.vertices=spial.vertices+.5*pialinnerdist;
end
end
...
x=mypcg(AtA,Atb,1e-10,7000,diag(AtA)+1e-3);
clear AtA Atb
load(d_tot_fname);delete(d_tot_fname);
...
gg=d_tot1*vv.img(mask_ind);clear d_tot;
...
save_nii(vv,[subbasename,'.mid.cortex.pvc.heat_newmid.nii.gz']);
vv.img(mask_ind)=(1./speed).*sqrt(1./(gr+eps));
% view_nii(vo);
vv.hdr.dime.datatype=64; vv.hdr.dime.bitpix=64;
save_nii(vv,[subbasename,'.mid.cortex.pvc.thickness_newmid.nii.gz']);
...
if ~exist('flag1','var')
    ind1=find((smid.attributes>6));
    refind1=find((smid.attributes<5));
    smid.attributes(ind1)=griddata(smid.vertices(refind1,1),smid.vertices(refind1,2),smid.vertices(refind1,3),smid.attributes(refind1),smid.vertices(ind1,1),smid.vertices(ind1,2),smid.vertices(ind1,3),'nearest');
end
if iso_heat==1
